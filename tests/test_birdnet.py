...
def test_extracts_timestamp_from_filename(tmp_path: Path) -> None:
    ...
    analyzer = BirdNetAudio({"lat": 48.746, "lon": 11.463}, str(audio_file))
...
def test_filter_detections_adds_metadata(tmp_path: Path) -> None:
    ...
    birdnet = BirdNetAudio({"lat": 48.746, "lon": 11.463}, str(audio_file))
    ...
    assert detections[0]["latitude"] == 48.746
    assert detections[0]["longitude"] == 11.463